...
#show: template.with(
  ...
  updated: datetime(year: 2025, month: 10, day: 3),
)
...
By far, we have assumed a Euclidean parameter space $Theta$ such that $forall theta, theta' in Theta$ the distance between two parameters $D(theta, theta')$ is given by
...
More generally, one can consider the parameter space as a _Riemannian manifold_. The (squared) distance between two points $theta$ and $theta'$ on a Riemannian manifold is
$ D(theta,theta') = (theta-theta')^top G(theta) (theta-theta'), $
where the _Riemannian metric tensor_ $G(theta) in RR^(d times d)$ is a square and positive definite matrix depending on $theta$. Note that the Euclidean space corresponds to a special case where $G(theta)=bold(I)$.
...
Particularly when $G(theta)$ is the Fisher information matrix (FIM), this is known as the _natural gradient descent_.
